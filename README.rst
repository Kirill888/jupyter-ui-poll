...
1. Display User Interface in Jupyter [#]_ using ``ipywdigets`` [#]_
...
And you want to be able to execute ``Run All`` menu option and still get correct output.
...
For a detailed overview see `Example notebook`_
...
process "execute cell" events, so ``Run All`` scenario will still be broken, as
code in lower cells will run before the data it operates on becomes available.
